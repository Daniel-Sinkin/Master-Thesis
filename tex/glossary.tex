...
\begin{description}[style=nextline, leftmargin=3.5cm, labelwidth=3.2cm]
  ...
  \item[Coalescing]
    The hardware mechanism by which memory requests from threads in a warp are
    serviced with one or more memory transactions. On recent CUDA
    architectures, effective coalescing is best understood as maximising the
    fraction of each transferred 32-byte segment that is actually used.
  ...
  \item[Warp]
    A group of 32 threads and the fundamental scheduling/SIMT unit on NVIDIA
    GPUs. On newer architectures, software should not assume implicit
    lock-step synchronisation across warp lanes.
