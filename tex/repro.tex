...
cuBLASDx (Preview) & 0.4.0 (provided via JSC MATHDx/25.06.0 module) \\
CUTLASS & 3.9.0 (provided via JSC MATHDx/25.06.0 module) \\
...
The NVIDIA driver (590.48.01) reports forward-compatibility with CUDA\,13.1;
however, all code was compiled against the CUDA\,12.6 toolkit. cuBLASDx was
acquired on this system through the JSC \texttt{MATHDx/25.06.0} module as the
cuBLASDx 0.4.0 preview package; NVIDIA documents that cuBLASDx is not shipped
with the CUDA Toolkit, so reproducing these experiments requires that separate
package source rather than a stock CUDA installation~\cite{cublas,nvidia-cublasdx-downloads}. CUTLASS and cuBLASDx require compute capability 8.0 (Ampere)
or higher. The current \texttt{-DCMAKE\_CUDA\_ARCHITECTURES=80} setting should
therefore be read as an Ampere-targeted build; running the same binary on newer
architectures depends on whether PTX and/or additional native targets are
embedded in the final binary~\cite{nvidia-ada-compat-guide}.
