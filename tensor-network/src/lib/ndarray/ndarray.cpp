...
auto NDArray::matrix(std::initializer_list<std::initializer_list<f64>> rows) -> NDArray {
    ...
    for (const auto &row : rows) {
        if (row.size() != col_count) {
            throw std::invalid_argument("NDArray::matrix requires all rows to have the same length.");
        }
...
auto NDArray::validity() const noexcept -> NDArrayValidity {
    ...
    if (shape_.empty()) {
        return data_.size() == 1 ? NDArrayValidity::valid : NDArrayValidity::scalar_storage_size_mismatch;
    }
    ...
    return data_.size() == expected_stride ? NDArrayValidity::valid : NDArrayValidity::data_size_mismatch;
}
