...
__global__ void hbm_stride_read_8byte_kernel(const std::uint64_t* in,
                                             std::uint64_t* out,
                                             std::size_t n_mask,
                                             int stride_elems,
                                             int loads_per_thread) {
  std::size_t tid = static_cast<std::size_t>(blockIdx.x) * blockDim.x + threadIdx.x;
  std::size_t idx = (tid * static_cast<std::size_t>(stride_elems)) & n_mask;

  std::uint64_t acc = 0;
#pragma unroll 1
  for (int i = 0; i < loads_per_thread; ++i) {
    acc ^= in[idx];
    idx = (idx + static_cast<std::size_t>(stride_elems)) & n_mask;
  }
...
int main(int argc, char** argv) {
  if (argc < 2) {
    std::fprintf(stderr,
                 "Usage: %s output_csv [runs warmup loads_per_thread threads_per_block "
                 "blocks_per_sm n_pow2]\n",
                 argv[0]);
    std::fprintf(stderr,
                 "Example: %s results/hbm_stride_raw.csv 10 5 4096 256 32 26\n",
                 argv[0]);
    return EXIT_FAILURE;
  }
  ...
  int loads_per_thread = (argc > 4) ? std::atoi(argv[4]) : 4096;
  int threads_per_block = (argc > 5) ? std::atoi(argv[5]) : 256;
  int blocks_per_sm = (argc > 6) ? std::atoi(argv[6]) : 32;
  int n_pow2 = (argc > 7) ? std::atoi(argv[7]) : 26;

  if (runs <= 0 || warmup < 0 || loads_per_thread <= 0 || threads_per_block <= 0 ||
  ...
  std::size_t n_elems = static_cast<std::size_t>(1) << n_pow2;
...

  {
    ...
    int init_blocks = static_cast<int>((n_elems + init_threads - 1) / init_threads);
  ...
  csv << "device,sm_count,threads_per_block,blocks,total_threads,loads_per_thread,"
         "warmup_runs,stride_bytes,stride_elements,run_idx,kernel_ms,read_bytes,"
         "achieved_bandwidth_gbps\n";

  std::size_t n_mask = n_elems - 1;
  double read_bytes = static_cast<double>(total_threads) *
                      static_cast<double>(loads_per_thread) *
                      static_cast<double>(sizeof(std::uint64_t));

  std::printf("Writing raw CSV to: %s\n", output_csv.c_str());
  std::printf("device=%s sm=%d blocks=%d threads=%d total_threads=%zu loads/thread=%d "
              "warmup=%d runs=%d working_set=%zu MiB\n",
              prop.name,
              sm_count,
              blocks,
              threads_per_block,
  ...
  for (int stride_bytes : kStrideBytes) {
    ...
    int stride_elems = stride_bytes / 8;

    auto launch = [&]() {
      hbm_stride_read_8byte_kernel<<<blocks, threads_per_block>>>(
          d_in, d_out, n_mask, stride_elems, loads_per_thread);
    };
    ...
    for (int run = 0; run < runs; ++run) {
      ...
      double bw_gbps = read_bytes / (static_cast<double>(ms) * 1.0e6);
