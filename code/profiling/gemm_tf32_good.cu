...
int main(int argc, char** argv) {
  ...
  std::printf("Case: GOOD (TF32 tensor-core path)\n");
  ...
  const double approx_a100_tf32_peak = 156.0;
  const double approx_peak_pct = 100.0 * tflops / approx_a100_tf32_peak;

  std::printf("Total time: %.3f ms\n", static_cast<double>(ms));
  std::printf("Avg GEMM:  %.3f ms\n", avg_ms);
  std::printf("Throughput: %.2f TFLOP/s\n", tflops);
  std::printf("Approx A100 TF32 peak: %.1f%% of 156 TFLOP/s\n", approx_peak_pct);
  std::printf("KPI target in Nsight Compute: tensor-pipe active should be HIGH.\n");
