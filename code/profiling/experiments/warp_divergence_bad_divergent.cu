...
__global__ void warp_divergent_branch(const float* in, float* out, int n, int inner_iters) {
  int tid = blockIdx.x * blockDim.x + threadIdx.x;
  if (tid < n) {
...
int main(int argc, char** argv) {
  ...
  std::printf("Case: BAD (intra-warp divergent branching)\n");
  ...
  auto launch = [&]() {
    warp_divergent_branch<<<blocks, threads>>>(d_in, d_out, n, inner_iters);
    std::swap(d_in, d_out);
  };
