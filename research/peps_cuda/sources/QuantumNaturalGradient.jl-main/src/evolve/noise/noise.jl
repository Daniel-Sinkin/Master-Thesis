...
mutable struct LangevinNoiseIntegrator <: AbstractCompositeIntegrator
...
function LangevinNoiseIntegrator(integrator::AbstractIntegrator, T_schedule::AbstractTemperatureSchedule;
    ...
    return LangevinNoiseIntegrator(integrator, T_schedule, θ_std, norm_preserving, L2_lr, verbose, history, history_view)
end

function LangevinNoiseIntegrator(integrator::AbstractIntegrator, T::Real; steps::Integer=1, final_T::Float64=0.9, kwargs...)
    T_schedule = TemperatureExpDecay(T, final_T, steps)
    return LangevinNoiseIntegrator(integrator, T_schedule; kwargs...)
end

function (integrator::LangevinNoiseIntegrator)(θ::AbstractVector, Oks_and_Eks_; kwargs...)
