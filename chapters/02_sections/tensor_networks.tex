...
This section treats tensor networks as an \emph{application source of
computational patterns}, not as a physics topic. Detailed notation and
model-specific physics choices are outside scope here and are introduced
separately with domain experts.

For high-performance computing, the relevant point is that tensor-network codes
repeatedly execute dense linear-algebra-like kernels on moderate to small
tensor blocks inside long iterative loops. Both per-kernel efficiency and
orchestration overhead therefore matter.
...
high-order object is factorised into a chain of local tensors. The relevant
point here is not the physics interpretation but the resulting compute
structure:
...
\subsection{Kernel Patterns Relevant to This Work}\label{sec:tn-kernel-motifs}
