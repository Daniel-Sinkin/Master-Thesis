...
with 128 physical SM partitions in the full design, while shipping A100 parts
expose 108 active SMs in the SXM4-40GB product configuration~\cite{nvidia2020a100,nvidia-ampere-in-depth}.
...
\cref{fig:ga100-n7-yield-curve} plots the resulting full-die-yield sensitivity.
...
\begin{figure}[H]
  ...
  \caption{First-order defect-limited yield sensitivity for a GA100-sized die
  ($A=8.26$\,cm$^2$) using $Y_{\mathrm{full}}\approx e^{-A D_0}$. Marker
  points use industry-reported N7 anchor values ($D_0\approx 0.33$ and
  $D_0\approx 0.09$).}
...
costs, and rely on NVLink for inter-GPU tensor redistribution whenever possible
