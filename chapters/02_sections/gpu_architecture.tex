...
For capacity accounting in this thesis, the A100 limit of \num{64} resident warps
...
this reason, the optimisation strategy in this thesis prioritises three levers:
...
\subsubsection{HBM Row/Column Access Path (Conceptual)}
\label{sec:hbm-row-column-path}

\begin{figure}[H]
  \centering
  \IfFileExists{images/HBMRowColumnSensePath.pdf}{
    \includegraphics[width=0.92\textwidth]{HBMRowColumnSensePath}
  }{
    \resizebox{0.92\textwidth}{!}{%
    \begin{tikzpicture}[>=Latex, line width=0.8pt, every node/.style={font=\small, align=center}]
      % Bank array and decoders
      \draw[fill=gray!12] (0,0) rectangle (10,6);
      \node at (5,5.7) {DRAM bank array (conceptual)};
      \draw[fill=blue!14] (-1.9,0) rectangle (-0.5,6);
      \node[rotate=90] at (-1.2,3) {Row decoder};
      \draw[fill=blue!14] (0,6.25) rectangle (10,7.2);
      \node at (5,6.72) {Column decoder};

      % Sense amps and I/O buffer
      \draw[fill=green!24] (0,5.35) rectangle (10,5.8);
      \node at (5,5.57) {Sense amplifiers (row buffer)};
      \draw[fill=gray!18] (10.45,5.05) rectangle (12.2,6.05);
      \node at (11.33,5.55) {Data\\buffers};
      \draw[<->] (10,5.55) -- (10.45,5.55);

      % Highlighted active row and transfer arrows
      \draw[fill=green!18] (0,2.45) rectangle (10,2.8);
      \node[anchor=west] at (0.1,2.62) {active row};
      \foreach \x in {0.7,1.4,2.1,2.8,3.5,4.2,4.9,5.6,6.3,7.0,7.7,8.4,9.1} {
        \draw[->,green!55!black] (\x,2.8) -- (\x,5.35);
      }

      % Bitfield annotation
      \node[anchor=west] at (-2.0,8.0) {Address split: \textcolor{green!45!black}{row bits} + \textcolor{blue!60!black}{column bits}};
      \draw[->,green!55!black] (1.8,7.85) -- (-1.2,6.0);
      \draw[->,blue!60!black] (5.8,7.85) -- (5.0,7.2);
    \end{tikzpicture}}
  }
  \caption{Placeholder schematic for HBM/DRAM row-column access (to be replaced
  by a final custom diagram). A read activates one row, latches it into sense
  amplifiers, then uses the column decoder to select transferred data.}
...
\begin{table}[H]
\centering
\caption{A100 KPI card (SXM4-40GB baseline) for performance modeling.}
...
Processing blocks (sub-partitions) per SM & \num{4} \\
Warp schedulers per processing block & \num{1} \\
Warp dispatch units per processing block & \num{1} \\
FP32 CUDA cores per processing block & \num{16} \\
FP64 CUDA cores per processing block & \num{8} \\
Tensor cores per processing block & \num{1} \\
FP32 CUDA cores per SM & \num{64} \\
FP64 CUDA cores per SM & \num{32} \\
Tensor cores per SM & \num{4} \\
Warp schedulers per SM & \num{4} \\
Warp dispatch units per SM & \num{4} \\
Maximum warps per processing block (derived) & \num{16} \\
Maximum threads per processing block (derived) & \num{512} \\
...
Total processing blocks on full GPU & \num{432} \\
Maximum resident warps on full GPU & \num{6912} \\
Maximum resident threads on full GPU & \num{221184} \\
\bottomrule
\end{tabularx}
\end{table}
...
\noindent\textbf{TPP definition used in this thesis.}
...
\begin{table}[H]
\centering
\caption{A100 throughput ceilings used in the thesis performance model.}
...
\begin{table}[H]
\centering
\caption{Derived A100 limits and capacity counts used throughout this thesis.}
\label{tab:a100-derived}
\begin{tabularx}{\textwidth}{X r}
\toprule
Metric & Value \\
\midrule
Peak FP32 performance per processing block (GFLOP/s) & \num{45.12} \\
Peak FP64 performance per processing block (GFLOP/s) & \num{22.56} \\
Peak FP32 performance per SM (GFLOP/s) & \num{180.48} \\
Peak FP64 performance per SM (GFLOP/s) & \num{90.24} \\
Tensor-core FP16 performance per SM (TFLOP/s) & \num{2.89} \\
...
Total FP64 cores & \num{3456} \\
Total processing blocks (sub-partitions) & \num{432} \\
Maximum resident warps per processing block (derived) & \num{16} \\
Maximum resident threads per processing block (derived) & \num{512} \\
Maximum resident warps on full GPU & \num{6912} \\
Maximum resident threads on full GPU & \num{221184} \\
