...
In practice, the entries of typical tensor contractions are not deliberately crafted to maximise the impact of truncation, and FP32 accumulation across many products reduces the effect of individual input truncations. For scientific kernels, this still requires workload-specific validation, but the underlying mixed-precision behavior is now well studied for tensor-core GEMMs and linear solves~\cite{markidis2018tensorcore-precision,haidar2018tensorcore-ir,fasi2023multiword-tensorcore}.
...
This single-rounding property is a core reason FMA-based formulations often have tighter forward error than separate multiply/add sequences~\cite{ieee754-2019,higham2002accuracy,jeannerod2017fma-complex}.
...
This property is directly relevant to tensor core computation: the multiply-accumulate $D = A \times B + C$ is implemented as a sequence of fused multiply-adds, and the single-rounding semantics mean that accumulated dot products are more accurate than a na\"ive loop of separate multiplies and additions. Recent tensor-core error analyses make this point explicit for matrix products and blocked accumulation schemes~\cite{fasi2023multiword-tensorcore}.
