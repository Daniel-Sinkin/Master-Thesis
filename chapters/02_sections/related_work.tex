...
especially important when available: it enables device-side GEMM composition
and supports launch-amortisation strategies that are difficult to implement
with host-only library calls~\cite{cublasdx}. NVIDIA documents cuBLASDx as a
preview package that is not shipped with the CUDA Toolkit, so any use of it in
this thesis must be treated as an explicitly version-pinned external
dependency~\cite{cublas,nvidia-cublasdx-downloads}. The launch-overhead benchmark in this thesis (\cref{sec:launch-overhead-benchmark}) uses this capability directly.

Custom kernels are evaluated against these libraries as
...
shares core HPC concerns with this work:
...
This work builds on those observations with a stricter focus on Ampere-era
...
This review covers work from the groups closest to the present project:
Wu, Di~Napoli, Rizzi, Waintal, Legeza, and their collaborators.
...
Menczer and Legeza report peer-reviewed JCTC results on massively parallel
tensor-network algorithms and accelerator-oriented implementations, with
complementary preprints for symmetry-aware and newest H100-focused
optimisations
(\cite{menczer2025massive-jctc,menczer2024ai-accelerators-jctc,menczer2023nonabelian,menczer2024efficient-dmrg-gpu}).

These studies report good scaling across heterogeneous CPU--GPU systems and
explicitly motivate mixed-precision/tensor-core execution for
contraction-heavy workloads
~\cite{menczer2025massive-jctc,menczer2024ai-accelerators-jctc,menczer2024efficient-dmrg-gpu}.
The direct transfer for this project is straightforward: reduce launch count for
...
regime~\cite{krinitsin2025ttn-ed}. A practical point for this project is their
...
\paragraph{Integration priorities.}
...
Four patterns show up repeatedly in this literature:
...
The table below is a working reference that translates paper-level findings
into CUDA optimisation actions for this project.

\begin{table}[H]
\centering
\caption{Working notes: transfer from selected literature to CUDA optimisation actions in the project.}
\label{tab:paper-transfer-notes}
\begin{tabularx}{\textwidth}{X X X}
\toprule
Paper (focus) & Reported methodology/result signal & Direct project action \\
\midrule
\cite{menczer2025massive-jctc} (massively parallel TN state algorithms on CPU--GPU systems) &
...
\cite{menczer2024ai-accelerators-jctc} (TN algorithms on AI accelerators) &
hardware-specific kernel organisation and execution strategy strongly influence effective performance, not just raw peak throughput &
...
\textbf{Why it matters.}
The same overlap pattern---launch compute, post an asynchronous collective,
gate the next kernel on the collective's completion---appears in the
distributed ChASE eigensolver~\cite{wu2022chase-gpu,wu2023chase-nccl} and
in Menczer et al.'s multi-GPU DMRG
implementation~\cite{menczer2025massive-jctc}.
