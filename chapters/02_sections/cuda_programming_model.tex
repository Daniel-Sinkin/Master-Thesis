...
CUDA (Compute Unified Device Architecture) is NVIDIA's parallel programming
platform for general-purpose computation on GPUs. A CUDA program combines host
code (CPU side) with \emph{kernels} launched on the GPU. Parallelism is
specified through a grid of thread blocks at launch time, and the hardware
schedules those blocks onto the available SMs.

\subsection{Thread Hierarchy and Kernel Launch}\label{sec:cuda-threads}

CUDA organises parallel execution into a three-level hierarchy:
\emph{grids}, \emph{thread blocks} (or simply \emph{blocks}), and
\emph{threads}. A kernel launch creates one grid, which is partitioned into
blocks. Each block contains a fixed number of threads that execute concurrently
on the same SM and can cooperate through shared memory and synchronisation
barriers. Threads in different blocks cannot synchronise with each other during
kernel execution.
...
The hierarchy in \cref{fig:cuda-thread-hierarchy} also includes \emph{warps}:
groups of 32 threads, which are the fundamental SIMT (Single Instruction,
Multiple Thread) execution unit on NVIDIA GPUs. For introductory explanations
it is common to describe a warp as executing one common instruction stream, but
on Volta and newer architectures independent thread scheduling means code must
not rely on implicit warp-synchronous progress unless the required
synchronisation is made explicit~\cite{nvidia-cuda-guide}.
...
\begin{equation}\label{eq:row-major-3d}
  \text{offset}(x,y,z)=((x\,s_y)+y)s_z+z=x\,s_y s_z+y\,s_z+z.
\end{equation}
...
\[
  \text{offset}(i_0,\dots,i_{d-1})=
  \sum_{m=0}^{d-1}
  i_m \prod_{n=m+1}^{d-1} D_n.
\]
...
\[
  \text{offset}(x,y,z)=x+s_x\,y+s_x s_y\,z.
\]
More generally, for rank-$d$ tensors in column-major order,
\[
  \text{offset}(i_0,\dots,i_{d-1})=
  \sum_{m=0}^{d-1}
  i_m \prod_{n=0}^{m-1} D_n,
\]
which is the column-major counterpart of the row-major formula above. In
\Cref{lst:transpose-2d}, the input is an $M \times N$ row-major matrix
(\texttt{in[row * N + col]}), and the output index
\texttt{out[col * M + row]} is simply the row-major index of the transposed
$N \times M$ output matrix. The kernel therefore stores the transpose; it does
not require switching to a different storage convention for the output array.

A concrete example: for a $4 \times 3$ matrix stored in row-major order, the element at row~2, column~1 maps to linear offset $2 \cdot 3 + 1 = 7$. This mapping determines whether warp-level memory accesses are coalesced or not (\cref{sec:coalescing}).

\subsection{Shared Memory and Synchronisation}\label{sec:shared-memory}

Shared memory is an on-chip, programmer-managed memory space visible to all threads within a block. It serves two purposes: as a software-managed cache to stage data from global memory, and as a communication channel between threads in the same block. Because shared memory is much lower latency than HBM-backed global memory, its effective use is often the difference between a bandwidth-bound kernel and one that approaches peak compute throughput.
...
The canonical use of shared memory is tiled (or blocked) matrix multiplication. A na\"ive matrix multiplication kernel that computes $C = AB$ with $A \in \R^{M \times K}$ and $B \in \R^{K \times N}$ has each thread compute one element of $C$ by reading an entire row of $A$ and column of $B$ from global memory. This results in $\BigO(K)$ global loads per thread. For the common FP32 per-output-element implementation, each thread performs about $2K$ FLOPs while reading about $2K$ FP32 input values ($8K$ bytes), so the arithmetic intensity is roughly $0.25\;\text{FLOPs}/\text{byte}$ before the output traffic is even counted---far below the A100-SXM4-40GB FP32 ridge point (\cref{tab:a100-derived}).
...
When a warp executes a load or store instruction, the hardware services the 32
thread addresses with one or more memory transactions. For devices of compute
capability 6.0 and newer, coalescing is described in terms of 32-byte memory
transactions: if the threads of a warp access adjacent 4-byte words, a fully
coalesced access is serviced by four 32-byte transactions~\cite{nvidia-cuda-best-practices,nvidia-cuda-guide}. The practical objective is to maximise the ratio of bytes requested by the warp to bytes transferred by the memory system. Strided or scattered patterns lower that ratio and therefore waste bandwidth even when the accesses are still legal.
...
In the implementation chapters, Nsight Compute profiles are used to validate
performance models, confirm that kernels operate in the expected regime
(compute-bound or memory-bound), and guide iterative optimisation of tensor
contraction kernels.
...
The details matter here because tensor network contractions launch many small
kernels, and the per-launch overhead of this pipeline adds up.
...
\subsubsection{MemcpyAsync vs Memcpy in Host--Device Timelines}
\label{sec:memcpy-async-vs-sync}

\Cref{fig:memcpy-async-timeline,fig:memcpy-sync-timeline} illustrate two common
execution patterns. The asynchronous version enqueues transfers and kernels and
lets the host continue with CPU work until an explicit synchronisation point.
The synchronous version uses blocking \texttt{cudaMemcpy}, which stalls the host
thread during each transfer.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.96\textwidth]{MemcpyAsync}
  \caption{Asynchronous host--device timeline using \texttt{cudaMemcpyAsync},
  streams, and event-based ordering. Host work overlaps queued device work
  until explicit synchronisation.}
  \label{fig:memcpy-async-timeline}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.96\textwidth]{MemcpySync}
  \caption{Synchronous host--device timeline using blocking
  \texttt{cudaMemcpy}. The host is stalled during H2D and D2H transfers.}
  \label{fig:memcpy-sync-timeline}
\end{figure}

The corresponding asynchronous CUDA pattern is shown below. For true asynchronous
host-device copies, host buffers should be page-locked (pinned), e.g.\ via
\texttt{cudaMallocHost}~\cite{nvidia-cuda-guide,nvidia-cuda-best-practices}.

\begin{lstlisting}[language=CUDA, caption={Asynchronous pattern with \texttt{cudaMemcpyAsync}, stream ordering, and explicit final synchronisation.}, label={lst:memcpy-async-pattern}]
cudaStream_t s1, s2;
cudaEvent_t done1;
cudaStreamCreate(&s1);
cudaStreamCreate(&s2);
...
// h_in / h_out should be pinned for true async copies.
...
bar();  // more host CPU work

cudaMemcpyAsync(h_out, d_out, bytes, cudaMemcpyDeviceToHost, s2);
cudaStreamSynchronize(s2);  // explicit synchronization point
\end{lstlisting}

\paragraph{Practical interpretation.}
The asynchronous pattern is usually preferred when host-side orchestration can
be overlapped with device execution. The synchronous pattern is simpler and
often acceptable for single-shot transfers, but it introduces avoidable host
idle time in pipelines with many transfer/compute stages.
