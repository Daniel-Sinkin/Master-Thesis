...
\section{GA100 Full Die Floorplan}\label{sec:ga100-floorplan-appendix}

\Cref{fig:ga100-full-landscape} reproduces the GA100 floorplan from the main
text (\cref{fig:ga100-full}) at full page width for legibility. All GPC,
memory-controller, L2, and NVLink/PCIe labels are readable at this scale.

\begin{landscape}
\begin{figure}[p]
  \centering
  \includegraphics[width=\linewidth]{GA100Full}
  \caption{Full GA100 floorplan schematic (landscape view). Graphics processing
  clusters (GPCs), L2 cache fabric, memory controllers, and NVLink/PCIe
  interfaces are shown. Compare with \cref{fig:ga100-full} in the main text
  and the per-SM detail in \cref{fig:sm-breakdown}.}
  \label{fig:ga100-full-landscape}
\end{figure}
\end{landscape}
...
\]
for selected allowances $k\in\{0,1,2,3,4,20\}$. The case $k=0$ recovers
$Y_{\mathrm{full}}$. The $k=20$ curve corresponds to the common A100 shipping
configuration with 108 active SM partitions out of 128.
...
\begin{figure}[H]
  ...
  (\SI{826}{\milli\meter\squared}) using harvest allowances
  $k\in\{0,1,2,3,4,20\}$ broken partitions out of 128.
  Marker points use industry-reported N7 anchor values
  ($D_0\approx 0.33$ and $D_0\approx 0.09$).}
...
\]
Under the same simplified partition model, allowing at most four broken
partitions raises the predicted values to about $86.5\%$ and $99.9\%$,
respectively. Allowing at most twenty broken partitions (the
108/128-SM case) saturates the model near $100\%$ at both anchor points.
