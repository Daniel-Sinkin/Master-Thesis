...
This approach avoids premature low-level tuning and keeps each optimisation
step attributable to a specific observed bottleneck.
...
This yields the following crossover points:
\[
...
In practical terms, small and medium contractions with low reuse remain
memory-bound unless layout/tiling raises intensity.

\subsubsection{Arithmetic-Intensity Landscape for Rectangular Kernels}
\label{sec:ai-landscape}

To study shape effects before full kernel implementation, we model a GEMM-like
kernel
\[
...
For the target shape regime in this thesis, we use $K=64$ and $t=16$ as a
representative setup. \Cref{fig:ai-heatmap-fp32,fig:ai-heatmap-fp64} map
...
\[
  S_{\infty}=\frac{1}{1-f}.
\]
This means that even very efficient kernels can be capped by serial orchestration,
launch overhead, synchronisation, and data-movement steps outside the parallel
kernel body.
...
For this thesis, these bounds are used as planning constraints: kernel-local
optimisation is necessary, but end-to-end scaling requires reducing serial
fractions in launch and orchestration paths.
