...
\begin{itemize}
  \item if $I_{\mathrm{meas}}<I^\star$ and $\eta$ is low, prioritise memory
    coalescing, reuse, and layout changes;
  \item if $I_{\mathrm{meas}}\ge I^\star$ and $\eta$ is low, prioritise
    instruction-level efficiency, occupancy/resource balance, and launch
    overhead reduction;
...
synchronisation, and data movement outside the parallel kernel
body~\cite{amdahl1967validity}.
