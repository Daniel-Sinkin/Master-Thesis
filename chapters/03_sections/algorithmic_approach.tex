...
\[
  ...
  I^\star_{\mathrm{FP32}} \approx \frac{19.5}{1.555}=12.5.
\]
...
For this project, roofline is used as a measurement workflow, not just an
analytical plot. The practical loop is:
\begin{enumerate}
...
\Cref{fig:ai-surface-fp32-padded} shows the padded FP32 landscape as a 3D
surface.
...
\begin{figure}[H]
  \centering
  \IfFileExists{images/arithmetic_intensity_surface_padded_fp32.pdf}{
    \includegraphics[width=0.82\textwidth]{arithmetic_intensity_surface_padded_fp32}
  }{
    \fbox{
      \parbox{0.8\textwidth}{
        Generate this figure with
  ...
  \caption{3D surface of effective arithmetic intensity
  $I_{\mathrm{eff}}(M,N,K;t)$ for FP32 at fixed $K=64$ with
  $t=16$ padding.}
  \label{fig:ai-surface-fp32-padded}
\end{figure}
...
\[
  S_{\infty}=\frac{1}{1-f}.
\]
Even efficient kernels are capped by serial orchestration overhead outside the
parallel kernel body, including launches, synchronisation points, and staging
traffic
