...
\begin{itemize}
  ...
  \item linked baseline libraries (cuBLAS, cuTENSOR, and the separately
    provided cuBLASDx preview package where used).
\end{itemize}

This metadata is logged with benchmark output so each result can be traced to a
specific binary configuration. When cuBLASDx is enabled, the recorded metadata
also includes its exact package version and acquisition source because NVIDIA
does not ship cuBLASDx as part of a stock CUDA Toolkit installation~\cite{cublas,nvidia-cublasdx-downloads}.
