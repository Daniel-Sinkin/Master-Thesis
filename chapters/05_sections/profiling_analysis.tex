...
\subsection{Irregular Contraction Training Run (SLURM Job 14507928)}
...
\begin{table}[t]
  ...
  \begin{tabularx}{\textwidth}{@{}X c c r r@{}}
    \toprule
    Variant & Warm-up & Timed iters & Avg.\ step time (ms) & Executed TFLOP/s \\
    \midrule
    Bad direct (serial launches) & 2 & 20  & 2.843314 & 0.020681 \\
    Good batched (one launch/step) & 20 & 200 & 0.039640 & 1.631807 \\
    \bottomrule
  \end{tabularx}
  \caption{Unprofiled timing summary for the irregular contraction training
    pair (job 14507928).}
  \label{tab:irregular-training-runtime}
\end{table}

Normalising by step time, the batched variant is $71.7\times$ faster, and its
executed throughput is $78.9\times$ higher.
...
\begin{table}[t]
  ...
  \begin{tabularx}{\textwidth}{@{}l >{\raggedleft\arraybackslash}p{2.2cm} >{\raggedleft\arraybackslash}p{2.2cm} >{\raggedleft\arraybackslash}p{2.2cm}@{}}
    ...
    \bottomrule
  \end{tabularx}
  \caption{Selected Nsight Compute counters for job 14507928. Direct and
