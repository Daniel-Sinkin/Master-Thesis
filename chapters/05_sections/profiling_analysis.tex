...
Nsight Compute data is used to explain \emph{why} performance changes in the
current preparatory experiments, not just \emph{how much} it changes.

\subsection{KPI Pack Used for Attribution}

To keep profiling interpretable and comparable, all Nsight Compute runs in this
thesis use a fixed ten-counter KPI pack. The profiling scripts in the companion
repository use exact metric names when available and otherwise fall back to
equivalent names for the installed Nsight Compute version. For K8, the intended
metric is the average-per-cycle form
\texttt{smsp\_\_warps\_eligible.avg.per\_cycle\_active}; sum-based variants are
not interpreted as the same quantity~\cite{nvidia-nsight-compute-cli}.
...
\begin{table}[t]
  ...
  \begin{tabularx}{\textwidth}{l X}
    ...
    K8 & \texttt{smsp\_\_warps\_eligible.avg.per\_cycle\_active} \\
...
\begin{description}
  ...
  In this thesis, only average-per-cycle forms are used for this
  interpretation; sum-based aggregates are treated separately because they can
  scale with aggregation scope and no longer read as ``eligible warps per
  scheduler cycle''~\cite{nvidia-nsight-compute-cli}.
...
\begin{enumerate}
  ...
  not dominant.
  \item \textbf{Memory-limited pattern:} K2 and/or K3 are high, K8/K9 often
  weaken when memory waiting dominates, and throughput scales weakly with more
  FMAs.
  ...
  \item \textbf{Scheduler starvation pattern:} K8 and K9 drop together, which
  suggests insufficient eligible warps or long-latency dependencies.
...
(K2 $16.54\%$ vs $3.62\%$).
These values are more consistent with issue/compute inefficiency than with an
occupancy limit: K6 remains high, while K5 and K9 are much weaker in the naive
kernel.
...
profile evidence comparable across optimisation steps. The pack is strongest at
supporting ``consistent with'' and ``suggests'' statements; it does not by
itself justify highly specific stall-cause claims without a deeper targeted
metric pass~\cite{nvidia-nsight-compute-profiling-guide}.

\subsection{Irregular Contraction Training Run}

The first end-to-end irregular pipeline run used
$\texttt{batch}=128$, $m=47$, $n=84$, and $k=64$ on one A100.
Two variants were compared:
\texttt{tn\_irregular\_direct\_bad\_seq} (serial launch regime) and
\texttt{tn\_irregular\_batched\_good} (batched launch regime).
...
\begin{table}[t]
  ...
  \begin{tabularx}{\textwidth}{@{}X r r@{}}
    ...
    \bottomrule
  \end{tabularx}
  \caption{Timing summary for the irregular contraction training pair.}
  \label{tab:irregular-training-runtime}
\end{table}

Normalising by step time, the batched variant is $71.7\times$ faster, and its
throughput is $78.9\times$ higher.

\begin{table}[t]
  \centering
  \small
  \begin{tabularx}{\textwidth}{@{}l >{\raggedleft\arraybackslash}p{2.2cm} >{\raggedleft\arraybackslash}p{2.2cm} >{\raggedleft\arraybackslash}p{2.2cm}@{}}
    \toprule
    ...
    \bottomrule
  \end{tabularx}
  \caption{Selected Nsight Compute counters for the irregular training pair. Direct and
  batched values are averages over two sampled launches.}
  \label{tab:irregular-training-kpi}
\end{table}

K8 is intentionally omitted from \cref{tab:irregular-training-kpi}: the
archived raw export appears to have used
\texttt{smsp\_\_warps\_eligible.sum.per\_cycle\_active}, and the resulting
values are too large to interpret as average eligible warps per scheduler
cycle. Re-extracting this metric with the average suffix is therefore treated
as a manual-review item rather than as evidence.

Three cautious observations stand out for later kernel work.
First, K10 is almost unchanged between direct and batched kernels
($86.14\%$ vs $86.01\%$), so divergence is not the main limiter in this case.
Second, K6 and K9 move sharply upward in the batched version, which is
consistent with much better scheduler feed and less underfilled execution in
the batched regime. Because K8 is currently withheld pending re-extraction,
this scheduler-starvation interpretation should be read as suggestive rather
than definitive.
Third, the repack kernel itself is not the pathological kernel in this run:
its K1/K5/K9 values are much higher than the direct contraction kernel.

The Nsight Systems summary from the same job is consistent with this view.
In the bad path, device-to-host copies total about $10.1$ MB across five copy
events, while the good path shows only one negligible device-to-host copy.
Combined with the timing gap in \cref{tab:irregular-training-runtime}, this
supports a simple rule for this workload class: keep irregular contractions
batched on device and avoid host-side synchronisation/copy steps inside the
hot loop.
...
\begin{figure}[t]
  ...
  \caption{Global-memory coalescing sweep over stride.}
