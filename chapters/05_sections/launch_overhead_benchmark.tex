...
quantifies the effect as a preparatory methodology-validation experiment and
tests whether kernel fusion is likely to matter for the workload class studied
here.
...
\begin{enumerate}
  \item \textbf{Separate launches:} each GEMM is dispatched as an individual
    cuBLASDx-based kernel, incurring the full launch overhead 1000 times.
...
In this project, cuBLASDx is a separately installed preview package rather than
part of a stock CUDA Toolkit installation; the exact package version and
acquisition path are pinned in \cref{app:reproducibility}~\cite{cublas,nvidia-cublasdx-downloads}.
...
\centering
\small
\caption{Total time for 1000 FP64 GEMMs ($n \times n$) on A100. \emph{Sep.}
  denotes 1000 kernel launches, \emph{Fuse.} denotes one fused kernel, and
  \emph{Overhd.} is the launch-overhead fraction of the separate-launch time.}
\label{tab:launch-overhead}
\begin{tabular}{@{}r r r r r@{}}
\toprule
$n$ & Sep.\ ($\si{\micro\second}$) & Fuse.\ ($\si{\micro\second}$) & Speedup & Overhd.\ (\%) \\
\midrule
...
In this preparatory pass, these results motivate treating launch reduction as a
first-order optimisation lever. Tensor network algorithms typically require many
contractions of modest size, which places them in the regime where launch
overhead can dominate. Fusing such contractions into a single kernel using
cuBLASDx's device-side GEMM API pays the launch cost once rather than once per
contraction. The design and implementation of the fused kernels are described
in \cref{sec:kernel-design}.
